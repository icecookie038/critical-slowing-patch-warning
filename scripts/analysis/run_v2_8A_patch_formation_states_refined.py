...
def first_sustained_onset(g: pd.DataFrame, state_col: str) -> float:
    ...
    if len(finite_x) < ONSET_PERSIST_K:
        return np.nan

    peak = np.nanmax(finite_x)

    if not np.isfinite(peak) or peak <= 0.05:
        return np.nan

    threshold = max(0.05, peak * ADAPTIVE_ONSET_FRACTION)

    active = np.isfinite(x) & (x >= threshold)
...
def plot_onset_summary(onset_summary: pd.DataFrame):
    ...
    plt.title(f"First sustained state activation, adaptive threshold={ADAPTIVE_ONSET_FRACTION}")
    plt.tight_layout()
