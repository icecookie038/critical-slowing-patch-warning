...
def clean_dataset(
    ...
    if critical_time is None:
        critical_time = time_idx + y_remaining
    ...
    mask = (
        ...
        & np.isfinite(y_risk)
        & np.isfinite(sim_id)
        & np.isfinite(time_idx)
        & np.isfinite(critical_time)
    )
    ...
    critical_time = critical_time[mask]
    sample_index = sample_index[mask]

    X_img = np.clip(X_img, 0.0, 1.0)
    y_remaining = np.maximum(y_remaining, 0.0)
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Direct path to v1.2 label-fix npz dataset. If provided, the script will load this file instead of generating a new dataset.",
    )
    ...
    if args.data_file is not None:
        ...
        print(f"\nLoading v1.2 dataset from: {dataset_path}")
        data = np.load(dataset_path, allow_pickle=True)

        required_keys = ["X_img", "X_patch", "y_remaining", "y_risk", "sim_id"]
        missing_keys = [k for k in required_keys if k not in data.files]

        if missing_keys:
            raise KeyError(f"Missing required keys in dataset: {missing_keys}")
        ...
        if "time_idx" in data.files:
            time_idx = data["time_idx"]

        if "critical_time" in data.files:
    ...
    y_pred = (risk_prob >= best_threshold).astype(int)

    model_name = f"CNN_GRU_{args.input_mode}"

    pred_df = pd.DataFrame({
        "sample_index": sample_index[val_idx],
        "sim_id": sim_id[val_idx],
        "time_idx": time_idx[val_idx],
        "critical_time": critical_time[val_idx],
        "y_remaining": y_remaining[val_idx],
        "y_true": y_risk_true.astype(int),
        "risk_prob": risk_prob,
        "y_pred": y_pred,
        "threshold": best_threshold,
        "model": model_name,
        "feature_mode": args.input_mode,
        "seed": args.seed,
        "split": "val",
    })

    pred_csv_path = result_dir / f"{model_name}_predictions.csv"
