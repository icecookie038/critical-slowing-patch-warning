...
- Historical v3.5–v3.7: real-data spatial-proxy and recovery audit. Recovery from
  the original cloud task is being reconciled; consult its recovery manifest and
  source verification status before citing an exact code version.
...
Freeze the current event IDs, raster footprints and block partitions. Compare
the recovered v3.7 spline cloglog baseline, M0, MQ, single-date spatial and QIR
with the same source-only fitting and calibration data. Check optimization on
source validation and use multiple fixed seeds. Report both directions and both
proper losses, with block-level uncertainty and dependence sensitivity. Audit
time-varying covariate availability before calling any result prospective.
